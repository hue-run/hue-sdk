...
| Bound lifecycle waits | Safe flush/shutdown return a failure result within the configured caller wait budget. Repeated calls share pending work. A timeout does not cancel a borrowed provider or prove delivery. Initialize Python after fork. |
...
| Keep capture deliberate | Capture is explicit; start metadata-only. Detached Python redactor inputs cannot mutate business arguments. External instrumentors, custom attributes and other exporters need their own capture policy. |

Behavioral evidence lives in [`sdk.test.ts`](packages/sdk-typescript/tests/sdk.test.ts), the installed-Node [`verify-safety.mjs`](packages/sdk-typescript/scripts/verify-safety.mjs), and Python's [`test_isolation.py`](packages/sdk-python/tests/test_isolation.py). Compatibility tests also exercise parentage, streaming, provider ownership, acknowledgements and actual encoded OTLP payloads. CI tests the installed tarball and independently installed wheels, including minimum/current supported Python versions; source-only tests do not establish package behavior.
