...
- Project service keys are preset-scoped. Use **Read and write**: it sends telemetry, reads delivery receipts, authorizes the evaluation, artifact and simulation APIs used by the optional clients and connects the [Hue MCP server](https://docs.hue.run/agents/mcp-server). **Tracing only** sends telemetry and reads delivery receipts, nothing more; **Read** reads the project through the MCP server and cannot send telemetry. The SDK clients do not browse stored traces; use the Hue app or the MCP server. These SDKs do not attach feedback or scores to stored traces. TypeScript's explicit `runSimulation()` helper orchestrates versioned simulation resources; it does not grant general read access or hide provider execution.
