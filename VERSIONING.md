...
- TypeScript: the runtime exports and exported types of `@hue-run/sdk`, `@hue-run/sdk/ai-sdk`, `@hue-run/sdk/environment`, `@hue-run/sdk/evals`, `@hue-run/sdk/managed` and `@hue-run/sdk/setup` and `@hue-run/sdk/evals/conversion-outcome-core.mjs`; the `hue` binary; and `@hue-run/sdk/setup-events.schema.json`.
...
silently substitute a different scorer for a frozen Scenario.
