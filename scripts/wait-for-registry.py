...
document `npm install` reads (with the dist-tag), and PyPI's simple index with pip's headers.
It never writes to a registry.
"""
...
def fetch_json(url: str, headers: dict[str, str]) -> dict | None:
    """The JSON document at `url`, or None when the registry does not serve it yet."""
