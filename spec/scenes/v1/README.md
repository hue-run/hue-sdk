...
Optional observation.externalSpanId is the current 16-hex OpenTelemetry span identity, for trace deep links; it does not affect matching or claim the result reached a model. Scene capturePolicy records {sourceContent:true,redactionVersion:"1"} for the initial policy; later policies use their own version. This policy is included in create/resume and frozen into each manifest.
