...
| Live spans | Placeholders for running Hue and AI spans, sent by the Hue transports from TypeScript `0.9.0` and Python `0.6.0` | Requires a Hue deployment that answers trace exports with `Hue-Pending-Spans: 1` (app.hue.run does). A receiver without that header, an older Hue or a generic collector, gets placeholders only in the first export that carries them: the SDK then records one warning and switches live spans off for that client. Turn `liveSpans` / `live_spans` off to send none at all. |
