...
| Python tracing and evaluations | Python 3.10 and 3.14 with OpenTelemetry 1.44.0, the certified combination recorded in `uv.lock`; Python 3.12 with OpenTelemetry 1.40.0, the declared floor | `opentelemetry-api`, `opentelemetry-sdk` and `opentelemetry-exporter-otlp-proto-http` are accepted as `>=1.40,<2`; releases inside the range that are not listed here are supported without a dedicated CI run. Local evaluation checkpoints require POSIX filesystem behavior. |
