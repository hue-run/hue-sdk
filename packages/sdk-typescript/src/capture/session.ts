...
export class CaptureSession {
  ...
  constructor(options: CaptureOptions) {
    ...
    this.bindings = options.sourceContent
      ? options.bindings.map((value) => {
          const sanitized = sanitize(value);
          if (sanitized.changed) this.dropped++;
          return captureRecord<Binding>("binding", sanitized.value);
        })
      : [];
    this.createBody = {
  ...
  /** Record explicit snapshots or journals with their actual causal boundary. */
  ...
  async observe<T>(
    ...
    try {
      try {
        ...
        if (
          !binding ||
          (binding.operations && !binding.operations.some((item) => item.name === operation))
        )
          throw new Error("Unselected operation");
  ...
  private async drain(deadline: number): Promise<CaptureReport> {
    ...
    try {
      ...
      while (this.queue.length) {
        ...
        if (!Number.isSafeInteger(result.captureRevision) || result.captureRevision < 0)
          throw new Error("Invalid capture response");
      ...
      try {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
          ...
          if (!this.finalization) {
            ...
            if (
              !Number.isSafeInteger(barrier.captureRevision) ||
              barrier.captureRevision < this.revision
            )
              throw new Error("Invalid capture barrier");
          ...
          try {
            ...
            if (
              result.revision !== pending.expectedCaptureRevision ||
              typeof result.digest !== "string" ||
              !/^[a-f0-9]{64}$/.test(result.digest) ||
              !Array.isArray(result.omissions) ||
              result.omissions.length > 100
            )
              throw new Error("Invalid finalization acknowledgement");
            result.omissions = result.omissions.map((value) =>
              captureRecord<Omission>("omission", value),
            );
          } catch (error) {
