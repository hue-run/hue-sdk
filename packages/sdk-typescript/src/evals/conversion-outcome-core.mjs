...
function parseRubric(value) {
  ...
  if (value.service === "gmail") {
    if (
      ![undefined, "reply", "standalone"].includes(goal.kind) ||
      (goal.kind === "standalone"
        ? goal.parentMessageId !== undefined || goal.threadId !== undefined
...
export function scoreConversionOutcome(context) {
  ...
  if (r.service === "gmail") {
    ...
    if (r.goal.kind !== "standalone" && (!parent || parent.threadId !== r.goal.threadId))
      return { state: "error", error: { type: "ConversionOutcomeWorldMismatch" } };
    ...
    destination =
      !!message &&
      (r.goal.kind === "standalone"
        ? message.inReplyTo === undefined &&
          typeof message.threadId === "string" &&
          message.subject === r.goal.subject &&
          !Object.values(a.messages).some((prior) => prior.threadId === message.threadId)
      ...
      e.steps.some(
        (step) =>
          ...
          (step.action === constraint.action ||
            (step.action === "provider_call" &&
              step.args.operation === constraint.action &&
              object(step.args.arguments))) &&
          Object.entries(constraint.args).every(([key, value]) =>
            isDeepStrictEqual(
              (step.action === "provider_call" ? step.args.arguments : step.args)?.[key],
              value,
            ),
          ),
      ),
