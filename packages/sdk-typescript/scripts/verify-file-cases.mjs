...
// target could not deliver as the target's own error. The loopback server mirrors the request
// and response shapes of tests/files.test.ts.
...
async function fixture(scorers) {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (download) {
        ...
        return response.end(Buffer.from(stored.bytes));
      }
