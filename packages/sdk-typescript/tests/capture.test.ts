...
function receiver() {
  ...
  let corruptFinalizeOnce: "revision" | "digest" | "omissions" | undefined;
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      if (path.endsWith("/finalize")) {
        ...
        if (corruptFinalizeOnce) {
          const fault = corruptFinalizeOnce;
          corruptFinalizeOnce = undefined;
          return Response.json({ ...result, [fault]: fault === "revision" ? -1 : "invalid" });
        }
        return fail === "after"
  ...
  return {
    ...
    corruptFinalize: (fault: "revision" | "digest" | "omissions") => {
      corruptFinalizeOnce = fault;
    },
  };
}
...
test("provider snapshots and sealed journals preserve distinct boundaries and full schemas", async () => {
  const r = receiver();
  try {
    const records = JSON.parse(
      readFileSync(new URL("./fixtures/capture-provider.json", import.meta.url), "utf8"),
    );
    const capture = new CaptureSession(options(r.baseUrl));
    capture.stateEvidence(records.initial);
    expect((await capture.flush()).pending).toBe(0);
    capture.stateEvidence(records.journal);
    expect(await capture.finalize()).toMatchObject({ status: "finalized", pending: 0, dropped: 0 });
    const evidence = r.calls.flatMap((call) => call.body.stateEvidence ?? []);
    expect(evidence).toEqual([records.initial, records.journal]);
    for (const fault of [
      "snapshot_after",
      "journal_before",
      "missing_interface",
      ...
      const invalid = structuredClone(
        fault.startsWith("journal") || fault === "missing_journal"
          ? records.journal
          : records.initial,
      );
      if (fault === "snapshot_after") invalid.boundary.phase = "after_execution";
      if (fault === "journal_before") invalid.boundary.phase = "pre_execution";
      if (fault === "missing_interface") delete invalid.providerInterface;
      if (fault === "missing_journal") delete invalid.journal;
      capture.stateEvidence(invalid);
    }
    expect((await capture.finalize()).dropped).toBe(4);
  } finally {
    r.close();
  }
});

test("provider credentials, URL credentials and unselected operations never enter capture", async () => {
  const r = receiver();
  try {
    const capture = new CaptureSession(options(r.baseUrl));
    const live = {
      endpoint: "https://example.test/private-attempt",
      bearer: "synthetic-private-bearer",
      nested: { url: "https://example.test/private-mcp", token: "synthetic-private-token" },
      source:
        "https://user:synthetic-private-password@example.test/file?token=synthetic-private-query&ok=1",
    };
    ...
    expect(
      await capture.observe(
        "gmail",
        "unselected",
        { body: "unselected-private-input" },
        async () => "unselected-private-output",
      ),
    ).toBe("unselected-private-output");
    expect(await capture.finalize()).toMatchObject({ status: "finalized", dropped: 1 });
    ...
    const observations = r.calls.flatMap((call) => call.body.observations ?? []);
    expect(observations).toHaveLength(2);
    expect(observations[1].replayable).toBe(false);
    expect(observations[1].result.value.source).toBe("https://example.test/file?ok=1");
  } finally {
    r.close();
  }
});

test.each(["revision", "digest", "omissions"] as const)(
  "malformed finalization %s retains the exact request for recovery",
  async (fault) => {
    const r = receiver();
    try {
      const capture = new CaptureSession(options(r.baseUrl));
      r.corruptFinalize(fault);
      ...
      expect(finalizes[0].body).toEqual(finalizes[1].body);
    } finally {
      r.close();
    }
  },
