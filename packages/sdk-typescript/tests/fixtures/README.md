...
# Planted-secret corpus

`planted-secrets.json` is generated by `planted-secrets.py` from a fixed seed: 2,800 synthetic
provider error texts, each with one to three random secrets planted where the error-text scrubber
redacts credentials. The cases come from fourteen families: key-value pairs, quoted and escaped JSON
values, `Authorization` headers, scheme words in prose, URLs, prefixed tokens, nested pairs (with or
without a quote, escaped or not, opening the nested value), scheme words glued to a value or ending
a credential, `API key:`, keys glued to a token and values between backslash-escaped single quotes.
Both suites check that no planted secret survives, case by case and with the whole corpus as one
text, within a time bound. Regenerate it with `python3 planted-secrets.py` after changing the
families.
