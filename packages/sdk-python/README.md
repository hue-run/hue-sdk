...
The placeholder then waits for the worker's next export like any queued span. The export delay is 1 s (64 queued records or `force_flush()` send sooner), so a placeholder typically reaches Hue about 1.5 s after its span starts. A placeholder whose span ends before it is exported is not sent, so a span shorter than about 1.5 s usually sends none and appears in Hue only when it finishes.
...
- Hue's processor builds each placeholder from the running span itself, before any `on_end` code runs. A span processor that scrubs or rewrites attributes in `on_end` before the span reaches Hue, such as a wrapper that forwards `on_start` to Hue, therefore does not run on placeholders: they carry the attributes the running span holds. Scrub content before it reaches the span instead. `redactor=` covers Hue's content helpers, because it runs when `set_input` or `set_output` sets the value, but it does not inspect other attributes. For instrumentor attributes, use the instrumentor's own capture controls, or `capture_content=False`, which strips recognized content keys from placeholders too. Otherwise pass `live_spans=False`.
...
- A processor wrapper that forwards `on_start` to Hue should also forward `on_end` for the same spans. A span that ends without reaching Hue is forgotten and its unsent placeholder discarded; one already sent can leave the span shown as running.
