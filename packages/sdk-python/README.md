...
| `span.record_file(role=..., media_type=..., data=... or sha256=...)` | `hue.file` event keyed by `hue.file.sha256`, with `hue.file.role` (`input`, `attachment` or `output`), `hue.file.media_type` and `hue.file.size`; `data` is hashed locally and never exported, and `hue.file.name` is recorded only when content is captured |
