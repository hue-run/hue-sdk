...
| `span.record_provider_tool_calls(response, request=..., servers=...)` | On a `model()` span: one `execute_tool {name}` child span per tool the provider ran itself (OpenAI Responses `mcp_call`, `web_search_call`, `file_search_call`, `code_interpreter_call`; Anthropic `mcp_tool_use` / `server_tool_use` with their results) with `gen_ai.tool.type=extension`, `gen_ai.tool.call.id`, `mcp.server.name`, `server.address` from the request's server URL, content-gated arguments and result, and `error.type` plus ERROR status on failure; an `mcp_list_tools` item becomes a `tools/list` span carrying that server's `gen_ai.tool.definitions` |
