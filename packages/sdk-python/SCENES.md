...
Every replay reserves a fresh trace ID and fresh occurrence cursors. Matching uses SHA-256 of canonical JSON containing the binding ID, operation, contract version and sanitized arguments. Repeated identical requests consume recorded occurrences in start order; distinct requests may reorder. An invocation consumes its selected occurrence even when its content cannot be downloaded or reconstructed; a later invocation advances to the next recorded occurrence. Start a fresh replay to reset all cursors. Overlapping identical calls with different results, or differently ordered outcomes from independent producers, are ambiguous and miss. An outer recorded tool bypasses its implementation and nested children. Selecting both a recorded ancestor and its descendant is rejected.
...
The path prefix is a literal string prefix: `/file` includes `/files/42`. Use `/files/` to select descendants of that directory while excluding `/files-extra` and the bare `/files` path. The same rule governs overlap rejection and playback ownership.
...
Consumed PDF, DOCX, PPTX and attachment responses are automatically linked as source files. HEAD responses retain file metadata without manufacturing a document body. HTTP playback validates bounded gzip/deflate decoding of shared raw recordings before exposing a response; unsupported content encodings miss explicitly. HTTP or MCP reconstruction failures also persist as misses, including when the caller catches them. Credential-bearing JSON response bodies, including supported compressed JSON, are ineligible and are omitted from the capture queue. Replay diagnoses partial-stream failures without turning them into complete recorded errors.
...
`SourceFile(data=...)` registers explicitly supplied source bytes; `SourceFile.from_path(path)` reads only that explicit local file. A URI by itself is a reference and is never fetched automatically. Relations are `query_attachment` and `tool_source`; `sources=lambda result: [...]` on a tool links extracted source files to the observed call. For async item streams, the extractor receives each consumed item. Sources from consumed items remain linked when the stream later closes or fails; unseen items are never pulled for extraction. Generated output documents are outside this feature.
