...
- Unreleased metadata: recommend one **Read and write** project key instead of **Tracing only**. The invite-only check asks whether the user has a Hue project and a **Read and write** key configured as `HUE_API_KEY`. That one key sends traces, verifies delivery, runs evaluations and connects the Hue MCP server, where it is configured as `HUE_MCP_KEY`; the fix for a rejected export is a **Read and write** key.
